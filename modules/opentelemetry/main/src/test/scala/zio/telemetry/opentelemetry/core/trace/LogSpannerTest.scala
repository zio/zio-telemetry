...
object LogSpannerTest extends ZIOSpecDefault {
    ...
    suite("hybrid backend")(
      test("creates both OTEL span and ZIO logSpan") {
        for {
          ...
          _             <- ZIO.scoped[Any] {
                             LogSpanner.installLogSpanner(OtelLogSpanner.makeHybrid(tracer)) *>
  ...
  private val scopingSuite =
    suite("scoping")(
          ...
          _             <- ZIO.scoped[Any] {
                             LogSpanner.installLogSpanner(OtelLogSpanner.make(tracer)) *>
                               ...
                               ZIO.scoped[Any] {
                                 ...
                                 LogSpanner.installLogSpanner(LogSpanner.default) *>
      ...
      test("OtelLogSpanner.layer installs correctly") {
        for {
          tracerTestkit <- ZIO.service[TracerTestkit]
          tracer        <- tracerTestkit.getTracer(instrumentationScopeName)
          _             <- ZIO.scoped[Any] {
                             OtelLogSpanner.layer.build.provideSome[Scope](ZLayer.succeed(tracer)) *>
  ...
  private val fiberCorrectnessSuite =
    suite("fiber correctness")(
      ...
      test("LogSpanner OTEL span survives timeout with ERROR status") {
        // Requires the uninterruptibleMask fix in Tracer.contextScope (#1069).
        // Without it, the span is recorded but with UNSET status because
        // statusMapper.handle is skipped during interruption.
        // Unskipped by #1069.
        for {
          tracerTestkit <- ZIO.service[TracerTestkit]
          tracer        <- tracerTestkit.getTracer(instrumentationScopeName)
          _             <- ZIO.scoped[Any] {
                             LogSpanner.installLogSpanner(OtelLogSpanner.make(tracer)) *>
                               (ZIO.sleep(1.hour) @@ LogSpanner.span("interrupted")).timeout(10.millis)
                           }
          spans         <- tracerTestkit.getFinishedSpans
          interrupted    = spans.find(_.name == "interrupted")
        } yield assert(interrupted)(isSome(anything)) &&
          assert(interrupted)(isSome(assertSpanStatusCode(equalTo(StatusCode.ERROR))))
