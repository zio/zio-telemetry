...
import zio.telemetry.opentracing.example.http.{ tracedBackend, Client }
...
object JaegerTracer {
  ...
  def makeService(
    host: String,
    serviceName: String,
    fiberRef: FiberRef[HttpOperation]
  ): ZLayer[Clock, Throwable, Client with Clock with OpenTracing] = {
    ...
    val opentracing = OpenTracing.fromEntrypoint(tracer, fiberRef, HttpOperation.entrypointExtractor)
