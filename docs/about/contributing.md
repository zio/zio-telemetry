...
After you [log into](https://github.com/login) GitHub using your account, go to the [ZIO Telemetry Project Page](https://github.com/zio/zio-telemetry), and click on [Fork](https://github.com/zio/zio-telemetry/fork) to fork the ZIO Telemetry repository into your own account.
...
mkdir zio
cd zio
git clone git@github.com:your-user-name/zio-telemetry.git .
```

If these steps were successful, then congratulations, you now have a complete copy of the ZIO Telemetry project!
...
The main project in ZIO Telemetry is `opentelemetry`, which you can focus on using sbt by issuing the following command:
...
You may have your own idea about what contributions to make to ZIO Telemetry, which is great! If you want to make sure the ZIO Telemetry contributors are open to your idea, you can [open an issue](https://github.com/zio-telemetry/issues/new) first on the ZIO Telemetry project site.

Otherwise, if you have no ideas about what to contribute, you can find a large supply of feature requests and bugs on the project's [issue tracker](https://github.com/zio/zio-telemetry/issues).

Issues are tagged with various labels, such as `good first issue`, which help you find issues that are a fit for you.

If some issue is confusing or you think you might need help, then just post a comment on the issue asking for help. Typically, the author of the issue will provide as much help as you need, and if the issue is critical, leading ZIO Telemetry contributors will probably step in to mentor you and give you a hand, making sure you understand the issue thoroughly.
...
Once you have an issue, the next step is to fix the bug or implement the feature. Since ZIO Telemetry is an open source project, there are no deadlines. Take your time!
...
```bash
git add core/src/shared/zio/zio-telemetry/NewFile.scala
```
...
Now while you were working on this great improvement, it's quite likely that other ZIO Telemetry contributors were making their own improvements. You need to pull all those improvements into your own code base to resolve any conflicts and make sure the changes all work well together.

To do that, use the `git pull` command:

```bash
git pull git@github.com:zio/zio-telemetry.git upstream
```
...
Next, [open a new pull request](https://github.com/zio/zio-telemetry/compare) on GitHub, and select _Compare Across Forks_. On the right hand side, choose your own fork of the ZIO Telemetry repository, in which you've been making your contribution.
...
Once you have a pull request open, it's still your job to get it merged! To get it merged, you need at least one core ZIO Telemetry contributor to approve the code.
