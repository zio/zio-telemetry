...
case class BackendHttpApp(tracing: Tracing, baggage: Baggage, contextStorage: ContextStorage) {
  ...
  val routes: HttpApp[Any, Throwable] =
    Http.collectZIO { case request @ Method.GET -> !! / "status" =>
      (for {
        _            <- baggage.extract(baggagePropagator, request.headers, getter)
      ...
      } yield response) @@ spanFrom(tracePropagator, request.headers, getter, "/status", SpanKind.SERVER)
    }

  def status: Task[Response] =
    for {
      proxyBaggage <- baggage.get("proxy-baggage")
      _            <- tracing.setAttribute("proxy-baggage", proxyBaggage.getOrElse("NO BAGGAGE"))
      _            <- tracing.addEvent("event from backend before response")
...
object BackendHttpApp {
  ...
  val live: URLayer[Tracing with Baggage with ContextStorage, BackendHttpApp] =
